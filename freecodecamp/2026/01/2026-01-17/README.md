...
This one was really about translating a “real world” representation (like "C2") into something I could do math with. Once I turned the column letter and row number into 0-based coordinates, the rest was just checking the knight’s 8 possible L-shaped offsets and counting which ones stay inside the board. The nice part is that this problem is naturally constant-time. No matter where the knight is, you only ever consider the same 8 moves, and the only difference is how many of those moves stay in bounds near the edges.
